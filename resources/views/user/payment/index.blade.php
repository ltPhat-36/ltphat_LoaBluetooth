    ...
    @else
        ...
        @if(!auth()->user()->hasVerifiedEmail())
            <div class="alert alert-warning">
                Bạn cần <a href="{{ route('verification.notice') }}">xác thực email</a> trước khi đặt hàng.
            </div>
        @else
            <form action="{{ route('payment.process') }}" method="POST" class="checkout-form">
                ...
                <div class="mb-3">
                    <p>Điểm hiện tại: <strong>{{ auth()->user()?->points ?? 0 }}</strong></p>
                    <p>Hạng thành viên: <strong>{{ auth()->user()?->level ?? 'Bronze' }}</strong></p>
                </div>
