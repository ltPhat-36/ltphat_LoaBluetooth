...
.banner-welcome{
    ...
    color:#fff;text-align:center;padding:60px 20px;border-radius:var(--radius);
...
.banner-welcome button{
    margin-top:24px;padding:14px 28px;font-size:1.1rem;
    background: linear-gradient(135deg,#f97316,#facc15);color:#1e293b;font-weight:700;
    border:none;border-radius:12px;box-shadow:0 8px 20px rgba(0,0,0,.2);cursor:pointer;
...
.product-card{
    ...
    box-shadow:var(--shadow);transition: transform var(--transition) cubic-bezier(.2,.8,.2,1),box-shadow var(--transition);
...
.fly-img {
  position: fixed;width: 120px;height: 120px;object-fit: cover;border-radius: 12px;
  z-index: 9999;pointer-events: none;transition: transform 0.8s ease, opacity 0.8s ease;
}

@media(max-width:900px){
    .qv-modal{grid-template-columns:1fr}
    .qv-left,.qv-right{padding:16px}
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  document.getElementById('qvClose').addEventListener('click',()=>qvBackdrop.classList.remove('show'));
  qvBackdrop.addEventListener('click',e=>{if(e.target.id==='qvBackdrop') qvBackdrop.classList.remove('show')});
  document.getElementById('qvWishlist').addEventListener('click',()=>{toggleWishlist(qvBackdrop.dataset.productId,document.getElementById('qvWishlist'))});
  document.getElementById('qvAdd').addEventListener('click',()=>{
    const id=qvBackdrop.dataset.productId;
    fetch("{{ url('/cart/add') }}/"+id,{method:'POST',headers:{'X-CSRF-TOKEN':'{{ csrf_token() }}','Content-Type':'application/json'},body:JSON.stringify({})})
    .finally(()=>{animateCartCount();qvBackdrop.classList.remove('show')});
  });
...
<div class="mb-4" data-aos="fade-down">
  <form action="{{ route('frontend.products.index') }}" method="GET" class="d-flex flex-wrap gap-2 align-items-center">
    
    {{-- Input tìm kiếm --}}
    <input type="text" name="q" class="form-control" placeholder="Tìm kiếm sản phẩm..." value="{{ request('q') }}"
      style="flex:1;min-width:180px;border-radius:12px;padding:10px 14px;border:1px solid #ddd;">

    {{-- Filter danh mục --}}
    <select name="category" class="form-select" style="width:auto;border-radius:12px;padding:10px 14px;">
    ...
    {{-- Sắp xếp --}}
    <select name="sort" class="form-select" style="width:auto;border-radius:12px;padding:10px 14px;">
    ...
    {{-- Nút tìm --}}
    <button type="submit" class="btn btn-accent" style="white-space:nowrap;padding:10px 20px;">🔍 Tìm</button>
  </form>
</div>
  ...
  <div id="productsSection">
    @foreach([['title'=>'Tất cả sản phẩm','products'=>$products]] as $section)
